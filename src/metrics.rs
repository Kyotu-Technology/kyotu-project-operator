use crate::{project_crd::Project, Error};
...
impl Default for Metrics {
    fn default() -> Self {
        let reconcile_duration = HistogramVec::new(
            histogram_opts!(
                "proj_controller_reconcile_duration_seconds",
        ...
        let failures = IntCounterVec::new(
            opts!(
                "proj_controller_reconciliation_errors_total",
        ...
        .unwrap();
        let reconciliations =
            IntCounter::new("proj_controller_reconciliations_total", "reconciliations").unwrap();
        Metrics {
            reconciliations,
            failures,
            reconcile_duration,
...
impl Metrics {
    ...
    pub fn reconcile_failure(&self, proj: &Project, e: &Error) {
        self.failures
            .with_label_values(&[
                proj.metadata.name.as_ref().unwrap().as_str(),
