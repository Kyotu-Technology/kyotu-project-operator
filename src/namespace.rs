...
//delete namespace
pub async fn delete_namespace(client: Client, name: &str) -> Result<String, Error> {
    let ns_api: Api<Namespace> = Api::all(client);
    //delete only if label app=kyotu-project-operator is present
    let res = ns_api.get(name).await?;
    let labels = res.metadata.labels.unwrap();
    ...
    if labels.get("app").unwrap_or(&"none".to_string()) != "kyotu-project-operator" {
        ...
        log::info!("Deleted namespace {}", name.to_string());
        Ok(name.to_string())
    }
}
